    ...
    this.Param = function Param(id, type, config, location) {
      ...
      function $$getDefaultValue() {
        ...
        if(defaultValue !== null && defaultValue !== undefined && !self.type.is(defaultValue))
          throw new Error("Default value (" + defaultValue + ") for parameter '" + self.id +
  ...
  function $StateProvider($urlRouterProvider, $urlMatcherFactory) {
    ...
    var stateBuilder = {
      ...
      params: function (state) {
        return state.parent && state.parent.params ? extend(state.parent.params.$$new(), state.ownParams) : new $$UMFP
          .ParamSet();
      },
    ...
    function registerState(state) {
      ...
      // Get parent name
      var parentName = (name.indexOf('.') !== -1) ? name.substring(0, name.lastIndexOf('.')) : (isString(state.parent)) ?
        state.parent : (isObject(state.parent) && isString(state.parent.name)) ? state.parent.name : '';
      ...
      $state.transitionTo = function transitionTo(to, toParams, options) {
        ...
        if(!options.reload) {
          ...
          if(options.reload && !reloadState) {
            throw new Error("No such reload state '" + (isString(options.reload) ? options.reload : options.reload
              .name) + "'");
          }
      ...
      function resolveState(state, params, paramsAreFiltered, inherited, dst, options) {
        ...
        function resolveViews() {
          ...
          forEach(state.views, function (view, name) {
            ...
            viewsPromises.push($resolve.resolve(injectables, dst.globals, dst.resolve, state).then(
              function (result) {
    ...
    function shouldSkipReload(to, toParams, from, fromParams, locals, options) {
      ...
      if(!options.reload && to === from &&
        (locals === from.locals || (to.self.reloadOnSearch === false && nonSearchParamsEqual(from, fromParams,
          toParams)))) {
        return true;
      }
    }
